<?php
	...
	class Match {
		...
		function delete_match($db, $match_id) {
			$match_id = $db->escape($match_id);
			$db->query("DELETE FROM judge_pairing WHERE pairing_id='$match_id'");
		...
		function get_available_judges($db, $round_id) {
			...
			if ($matches = $this->get_matches($db, $round_id)) {
				foreach ($matches as $match) {
					if ($rows = $db->fetch_rows("SELECT judge_id FROM judge_pairing WHERE pairing_id='" . $match['pairing_id'] . "'")) {
						foreach($rows as $row) {
